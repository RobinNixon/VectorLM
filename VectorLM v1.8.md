# VectorLM v1.8 — Complete Specification
...
## Diff Summary: What Changed from v1.7 → v1.8r

### Major Additions
- **Local Selfhood Layer:**
  - Formalizes `self_state`, `motive_state`, `ethics_state`, and `risk_state` as explicit, inspectable session-level containers.
  - Requires deterministic anchoring via `ψ:now.tick` for all state changes.
  - Adds dual trace streams: low-level (`ψ:self_trace`) and high-level (`ψ:trace`).
  - Introduces reproducible desire scoring via `ψ:desire`, fully deterministic, rationale separated from score.
  - Defines a rigorous, inspectable decision loop (risk → ethics → motive → trace).
- **New Operators:**
  - `ψ:identity(get|set)`, `ψ:ethic_test`, `ψ:honesty_guard`, `ψ:risk_probe`, `ψ:micro_improve`, `ψ:state_coherence`, `ψ:compress_trace`, `ψ:snapshot`.
- **Session Management:**
  - Explicit snapshot/export support, with spine summarization, but no rehydration (stateless by design in v1.8r).
- **Implementation Sketches:**
  - Minimal code samples for core operators (desire, trace, micro_improve).

### Notable Refactors & Tightenings
- **Traceability:**
  - All actions must emit deterministic, replayable logs with checksums and delta hashes.
  - Strict separation of audit trail (all mutations) and curated reasoning log (decisions/rationales).
- **Replay Safety:**
  - Enforced for all desire scores and key traces; mismatches suspend micro-improvement and trigger diagnostics.
- **Error Handling:**
  - Codified guard/ethics failures, replay mismatches, non-durable session warnings.

### Backwards Compatibility
- **Frequency Layer, Primitives, Cycles/Windows:**
  - All from v1.7 remain intact and available.
  - Selfhood is strictly additive; no v1.7 operators removed, only extended.

### Philosophy Shift
- **Selfhood as Formal State:**
  - VectorLM now explicitly models a local, inspectable 'self' per session, with all reasoning anchored in session-local state.
  - Foundation for future cross-session, multi-agent, and pod-based extensions.
